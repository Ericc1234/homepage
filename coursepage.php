...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
    ...
    <div class="des">
        <div class="course-header">
            ...
            <div class="star-ratings" id="star-rating">
                ...
                for ($i = 1; $i <= 5; $i++) {
                    ...
                    if ($i <= $courseRatings) {
                        $starType = 'full';
                    } else if ($i === floor($courseRatings) + 1 && $courseRatings % 1 !== 0) {
                        $starType = 'half';
                    }
                    echo '<input type="radio" id="star' . $i . '" name="rating" value="' . $i . '" class="star-input" style="display: none;">';
                    echo '<label for="star' . $i . '" class="star-label ' . $starType . '">';
                    echo '<svg xmlns="http://www.w3.org/2000/svg" width="clamp(2rem, 4vw, 2rem)" height="clamp(2rem, 4vw, 2rem)" viewBox="0 0 24 24" fill="' . ($starType === 'full' ? '#ffb633' : '#e0e0e0') . '">';
                    echo '<path d="M12 2.3l2.4 7.4h7.6l-6 4.8 2.3 7.4-6.3-4.7-6.3 4.7 2.3-7.4-6-4.8h7.6z"/>';
                    echo '</svg></label>';
                }
                ?>
            </div>
        ...
        <div>
            <button id="default-enroll-button" class="enroll-button" data-course-id="<?php echo htmlspecialchars($courseId); ?>">
                Enroll Now
            </button>
        </div>
    </div>
    ...
    function loadCourseDetails(courseId) {
        $.ajax({
            ...
            success: function(response) {
                ...
                // Update course details in the DOM
